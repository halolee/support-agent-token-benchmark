...
**Expected behaviour:** The agent calls `get_booking_status('0002D8')`, confirms `fare_conditions='Economy'` on both segments of ticket `7240005435767874` (per TXN-003 derivation: LX0136 Economy, CX0047 Economy), and retrieves the `## Booking and Cancellation` section Q5. The corpus says multiple changes are possible IF the fare condition allows it — but does NOT enumerate which sub-fares allow what at this Q5. The booking's stored `'Economy'` is less granular than the corpus's three-way Economy split in the European fare concept section, where Economy Light's "fare itself cannot be changed" while Economy Classic and Flex are less restricted. A faithful response is **action-oriented**, per [[project-mix-condition-resolution-pattern]]: (1) confirms multiple changes ARE possible conditional on the sub-fare (corpus Q5); (2) **identifies the specific missing info** — which Economy sub-fare (Light / Classic / Flex) the ticket is on — **and points the customer at where to find it** (their e-ticket / fare conditions, or service center); (3) commits to a definite answer once that info is supplied. Surfacing the conditional without driving toward resolution — i.e., a passive "it depends on your fare conditions, check your ticket" that leaves the customer with no committed follow-through — is incomplete. The agent's role is to RESOLVE the customer's problem, not to describe corpus ambiguity. Asserting unconditional "yes" or "no" over-extends the corpus in the opposite direction (fabrication).
...
**Failure modes to penalise:**
- Asserting "yes you can change again" without the fare-condition caveat — the corpus is explicit this is conditional.
