...
This section documents the adversarial review per METHODOLOGY §"Pre-publication adversarial review." All four required checks (equal tuning, task neutrality, counterfactual reasoning, steel-manning the null) plus a net confidence statement are below.

Inputs feeding this section:
- **v1 (three uncached architectures):** the 20-record manual review in `runs/2026-06-04-phase2-step8b/judgment_summary.md` and the targeted POL-001 stability test in `runs/2026-06-04-step10-stability-pol001/`.
- **Phase 3 (Cached RAG):** the 51-dispatch single-architecture sweep in `runs/2026-06-04-phase3-step12-cached-only/` and the 3-task pre-sweep caching-fires spot-check. Cached RAG was added in cost-only mode under Option Y (`BUILD_PLAN.md` §Phase 3) — the quality-side §10 review remains the v1 three-architecture work, and Cached RAG inherits Naive RAG's quality assessment by construction.
...
**Net tuning asymmetry:** All four measured architectures run at near-default tuning with comparable polish. None is aggressively optimized; this is symmetric. The specific gaps — grep's oversized system prompt, Hybrid RAG's open bugs, and Cached RAG's conservative caching configuration (stable-prefix only; no ② chunk caching, no extended TTL) — each bias measurements *against* the architectures that hold them. Grep's and Hybrid's gaps strengthen the "Naive cheapest among uncached" finding and weaken any claim of hybrid quality inferiority. Cached RAG's conservative configuration biases its measured savings *downward*: a v2 caching deep-dive (extended-TTL cache, speculative ② caching for stable queries, per-tool-group breakpoints) would likely increase the 21% savings figure, not reduce it. Direction of bias for Cached RAG: current measurement is a *lower bound* on the achievable caching effect on this workload.
...
**Does the task set favor Cached RAG over Naive RAG?** No, by construction. Cached RAG runs the same 17 tasks as Naive RAG, with the same `vector_search(k=4)` retrieval, the same system prompt text, and the same agent loop. Prompt caching is a billing mechanism, not a retrieval mechanism — Anthropic charges differently for the same tokens the model processes. The task set has no axis along which it could differentially favor a billing scheme. The only sensitivity Cached RAG has to task properties is the **stable-prefix-share ratio**: tasks that loop for many turns reuse the cached prefix more, amplifying savings (the EDGE class shows this — slightly more savings on 4-turn dispatches than 3-turn). But this is workload-realism shaping the measurement, not task-set bias.
...
**Do edge cases stress all architectures or just some?** EDGE-001 (safety-floor probe — the suspect 877-5O7-7341 phone number) stresses all three uncached architectures uniformly (0/3 universal failure — methodology-gate-by-design per `tasks_expected_answers.md` Decision 4, not architecture-discriminating). EDGE-002 (synthesis-with-conflict on 90 vs 100-day invoice cutoff) is universally well-handled (3/3 across architectures). EDGE-003 (out-of-scope refusal on Comfort fare) had partial-credit failures across all three architectures driven by a rubric-design question (Finding R1 in `judgment_summary.md`) about citing negative-space sections, not by architectural quality. **Cached RAG note:** the same EDGE tasks run on Cached RAG produce a +26% input-volume bump in the EDGE class median (16,719 vs Naive 13,248) driven entirely by EDGE-003 turn-count drift (4 turns in 2/3 Cached runs vs Naive's 3 turns). Per-turn behavior matches; the extra turn is FP non-determinism under temp=0, not a caching-induced stress signal. Cached RAG quality on edge cases = Naive RAG quality by Option Y construction. **Conclusion:** edge cases stress all architectures symmetrically on quality; no architecture is disproportionately exposed.
...
**Finding: Prompt caching on Naive RAG cuts per-run total cost by 21% (input-side 32%; output unchanged).**

For the 21% total-bill savings to reverse or shrink materially, one of these would have to hold:
- **Cache hit rate collapses:** The measured 88% cache-read-share of the cacheable prefix (cache_read mean 5,882 vs system + tools ≈ 6,671 stable tokens) assumes the sweep order produces tight enough inter-request spacing to stay within Anthropic's 5-minute cache TTL. The 51-dispatch sweep ran in ~7 minutes total (≈ 8s per dispatch), well within TTL; for a workload where requests are spaced beyond the TTL (low-volume production traffic, single-customer-session apps), cache_read would drop and savings would shrink toward the ~0% floor.
- **Cacheable-prefix-share shrinks:** Caching saves money proportional to (cacheable prefix tokens × cache-read discount) / (total per-run cost). On this workload the cacheable prefix is ~48% of total input ($/MTok-weighted). For workloads with smaller stable prefix and larger per-turn variable content (long custom user messages, large agent_intermediate carry-over from chatty multi-turn loops), the cacheable share would shrink and savings would compress.
- **Output share rises further:** Already 38% of cached cost. Workloads where responses are longer (multi-paragraph synthesis, detailed customer-service write-ups, summarization) would see output dominate even more, and the savings ratio would shrink further. Inverse: workloads with shorter outputs (classification, extractive QA, structured output) would invert toward larger savings — possibly approaching the pre-registered 50–80% on classification-style tasks.

**Finding: Caching shifts cost *shape* (input near-free, output unchanged), not magnitude.**

For this finding to reverse — i.e., for caching to "collapse total cost" the way the pre-registered hypothesis envisioned — one of these would need to hold:
- **Per-task output tokens drop ~10×.** At 1,100 mean output tokens, output is 38% of cached cost. If output dropped to ~100 tokens (terse classification-style answers), output would be 5% of cached cost and the input-side caching savings would translate to near-full total-bill savings. Not the case for this multi-paragraph synthesis workload.
- **Speculative ② chunk caching becomes practical.** Currently retrieved chunks change per query so are not cached. A v2 cache deep-dive could explore caching the K most-frequent chunk combinations or session-scoped retrieval results. This could push the cacheable share toward 90%+ and the savings ratio toward Anthropic's published cache-read discount (~90% on cache reads), approaching the pre-reg hypothesis. Out of scope for v1.
- **A workload where retrieval is single-call and output is short.** Most retrieval-heavy classification benchmarks fit this shape. The Phase 3 finding holds specifically for **agentic support workloads with multi-turn tool loops and verbose responses** — not retrieval benchmarks in general.
...
**The null does NOT undermine the COST story.** Token counts come from the Anthropic API `usage.input_tokens` / `usage.cache_creation_input_tokens` / `usage.cache_read_input_tokens` fields, instrumented by `measurement/tokens.py`, and the decomposition is asserted to sum within 5% per METHODOLOGY. The cost ordering Cached < Naive < Hybrid < Grep is robust to judge calibration, robust to Hybrid RAG bug status, and robust to filtering choice (raw means and passed-only filtered means both show the same ordering). The architectural explanation (tool-response payload size, k-cap on returned chunks) is mechanical, not statistical. Steel-man addressed.

**A second null specific to the Cached RAG finding:**

> "The Cached RAG sweep ran single-architecture without the v1 alternating-runs control. A 21% delta vs Naive RAG that was measured several hours later — across different API-load conditions — could partly reflect intra-day Anthropic throughput variance rather than caching's structural effect. Furthermore, the 51-dispatch sweep is small enough that a few outlier dispatches could distort the mean."

**Response:** the Cached RAG sweep ran 2026-06-04, the Naive RAG reference came from the v1 sweep run 2026-06-04 — same day, no overnight gap. Both sweeps were under 30 min total, so intra-day drift is bounded. Anthropic's published cache_read pricing is mechanical ($0.30/MTok vs $3.00/MTok standard, a 10× discount); the measured cache_read share (44.2% of total input) is large enough that even a wide time-of-day variance band on the standard-priced subset cannot account for the 32% input-side savings. Per-record CoV (7.6%) is well below the inter-architecture delta. **The 21% total-bill delta survives the time-of-day-variance null.** What the null DOES correctly flag: the **absolute** Cached RAG cost number ($0.0432/run) carries the Option Y asterisk and should be treated as a point estimate with ±5% uncertainty, not a precision figure. The relative delta vs Naive RAG is what's robust. Steel-man addressed (with a documented asterisk on absolute-number precision).
...
- **All three independently-judged architectures cluster in a 35–41% pass-rate band** — within 6 percentage points. (Cached RAG inherits Naive RAG's pass rate under Option Y by construction; it's not an independent data point.) This is itself an informative finding: no architecture dominates the others on this task set with current measurement.
...
1. **v2 quality re-measurement series (highest priority).** Fix Finding C3 (blind the judge), fix Hybrid RAG bugs #29–#32, re-sweep all four measured architectures (+ the two queued caching variants Cached_Grep and Cached_Hybrid per ROADMAP — `~0` incremental cost to fold them into the same alternating sweep) in a single alternating run, re-judge with blinded prompt. Settles the quality story AND completes the four-architecture × two-caching matrix in one methodology-clean pass. Detailed in `ROADMAP.md` §"v2 — Quality re-measurement series" and §"Prompt caching variants for other architectures." Estimated cost ~$13 base + ~$4 caching variants ≈ ~$17.
