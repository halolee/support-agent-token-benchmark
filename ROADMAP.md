...
v1 measures Naive RAG and Cached RAG. Future work: measure Hybrid RAG with caching, Grep search with caching — the remaining two cells of the cache matrix in `comparison.md` §"Scope of measurement."

- **Original trigger condition (pre-Phase 3):** "If Cached RAG shows dramatic cost collapse, the question 'does caching collapse cost for non-A architectures too' becomes important." — **NOT MET.** Phase 3 measured 21% total-bill savings, which is meaningful but does not match the "dramatic collapse" framing.
- **Revised trigger (post-Phase 3, 2026-06-04):** the v2 quality re-measurement re-sweep is going to alternating-sweep all four architectures anyway (for the blinded-judge methodology fix); adding Cached_Grep and Cached_Hybrid to that same alternation is **~0 incremental day** of work and completes the four-architecture × two-caching matrix in a methodology-clean single run. The cost-benefit threshold moved because the marginal cost dropped, not because the original "dramatic collapse" finding materialized. This is a deliberate trigger-condition revision, not a silent bypass — recording the move explicitly so future readers can reconstruct the decision.
- **Article-worthy v2 sub-question:** does Grep search's larger stable prefix (~2,601-token system prompt vs Naive's 1,693) give caching more leverage? Does Hybrid RAG's larger tool-output payload (k=6 reranked chunks) interact with caching differently? The Phase 3 finding suggests caching effect is bounded by the cacheable-prefix-share-of-total-input ratio; architectures with larger stable prefixes (Grep) or smaller output share would show larger savings.
- **Estimated additional work:** ~0.5 day per architecture if measured standalone; **~0 incremental day** if folded into the v2 quality re-measurement re-sweep. Recommended path: fold into v2.
...
| 2026-06-04 | Phase 3 ship: Cached RAG via Option Y (cost-only, no re-judge) — choose Y over X (full 4-arch re-sweep with re-judge) | Cached RAG is conceptually Naive RAG with prompt caching enabled; caching changes input-token *pricing*, not the tokens the model sees. Sonnet 4.6 at temp=0.0 is close-to-deterministic on identical inputs, so quality is asserted equivalent to Naive RAG by construction (3-task spot-check confirmed `cache_read > 0`, matching tool sequences, semantically equivalent responses). A non-blinded re-judge under current Finding C3 would produce numbers the v2 quality re-measurement series supersedes anyway, burning ~$5 for ephemeral results. Option Y (51 dispatches, single-arch, ~$2.20 actual) ships the cost finding cleanly with an explicit Option Y asterisk in §"Confidence and known biases" (single-arch sweep, no alternation; quality inherited from Naive RAG). Option X (full 4-arch alternating re-sweep with re-judge) deferred to v2 where the blinded-judge protocol + Hybrid RAG bug fixes carry the methodology-clean re-measurement in a single pass. **Article-worthy Phase 3 finding:** Caching shifts cost *shape* (21% total-bill savings, 32% input-side; output unchanged), not magnitude — the pre-registered 50–80% input-cost-reduction hypothesis over-anchored on cache-read pricing without modeling the un-cached output share. Tag `cached-rag-v1` parallel to `measurement-v1`. |
