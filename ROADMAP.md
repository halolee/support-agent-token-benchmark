...
- **Trigger to add:** Phase 3 measured a ~21% total-bill savings from caching on Naive RAG — meaningful but not the "dramatic collapse" the original trigger condition contemplated. The natural v2 question is whether Grep search's larger stable prefix (oversized system prompt ~2,601 tokens vs Naive's 1,693) gives caching more leverage, and whether Hybrid RAG's larger tool-output payload (k=6 reranked chunks) interacts with caching differently. Most informative path: package the two caching variants into the v2 quality re-measurement re-sweep so the four-architecture × two-caching matrix is complete in one alternating run (cost-clean + quality-clean), rather than two separate one-architecture sweeps.
- **Estimated additional work:** ~0.5 day per architecture if measured standalone; **~0 incremental day** if folded into the v2 quality re-measurement re-sweep (Cached_Grep and Cached_Hybrid added as additional architectures in the alternating sweep). Recommended path: fold into v2.
