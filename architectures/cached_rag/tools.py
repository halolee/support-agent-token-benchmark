"""Cached RAG tool definitions.

Identical to Naive RAG's tool definitions except for a single cache_control
marker attached to the last entry of TOOL_SCHEMAS (see below). The marker
makes the entire tool definitions block a cache breakpoint, so the ~4.7K
tokens of tool overhead measured for Naive RAG in v1 are billed at cache-
read pricing (~10% of input price) on every turn after the first call.
...
ONLY through Support Content's `vector_search` tool. This module:
  ...
  - defines `vector_search` (Support Content's interface; identical to
    Naive RAG's),
  - re-exports the booking tools (Booking Systems — identical across
    architectures) and `audit_log` (Compliance — identical across
    architectures, with a Cached-RAG-only cache_control marker copied
    onto the schema; the shared schema is not mutated).
...
# Cached RAG marks a cache breakpoint at the end of the tools list.
# Anthropic's prompt caching caches everything from the start of the
# tools array up to and including any block with `cache_control` set, so
# attaching the marker to the LAST entry caches all tool definitions as
# a single ~4.7K-token prefix block (per v1 Naive RAG tool_overhead
# measurement). We attach to a copy of the shared AUDIT_TOOL_SCHEMA so
# the shared schema (used by Naive RAG, Grep search, Hybrid RAG) is not
# mutated; only this architecture sees the cache_control marker.
