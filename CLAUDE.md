...
A measurement framework, not a product. Four reference customer-support agents (Naive RAG, Cached RAG, Grep search, Hybrid RAG) are implemented against the *same* corpus, *same* tasks, and *same* model so per-task token usage can be compared head-to-head under enterprise-realistic constraints. The headline artifact is `measurement/results/comparison.md`. Two further architectures (Bounded tools, Stuffed corpus) are scoped but deferred to v2; see `ROADMAP.md`. The companion LinkedIn article uses a lettered taxonomy (A/A+G/B/C/D/E/F/H/I) for cross-reference; the repo uses descriptive names directly and maps the letters once in `README.md`.
...
The repo is currently **docs and scaffolding only** — no Python implementation has landed yet. `BUILD_PLAN.md` is the authoritative sequencing guide: Phase 1 = foundation + token accounting, Phase 2 = Naive RAG, Grep search, Hybrid RAG + adversarial review, Phase 3 = Cached RAG (caching variant), Phase 4 = deferred Bounded tools / Stuffed corpus. Implement in that order — `measurement/tokens.py` is load-bearing for all architectures and must exist before any agent.
...
- **Embedding model:** `BAAI/bge-m3` (MIT-licensed, self-hosted) for Naive RAG, Cached RAG, and Hybrid RAG. No third-party embedding API. The choice eliminates data egress exposure; see `METHODOLOGY.md` §"API vs self-hosted choices."
- **Caching:** Disabled for Naive RAG, Grep search, Hybrid RAG. Enabled for Cached RAG (which exists specifically to measure the caching effect).
...
- **Token decomposition** (Silicon Data 5-category + `agent_intermediate` for multi-turn = 6 input-side categories) must sum to API-reported `input_tokens` within 5%. `measurement/runner.py` asserts this and flags discrepancies; do not silence the assertion. See METHODOLOGY §"Multi-turn extension" for why the sixth category exists.
- **System prompt token targets:** Naive RAG ~500 tokens, Grep search ~300-400 tokens, Hybrid RAG ~500 tokens. These are part of the design, not accidental.
...
- **Runs:** Three runs per task per architecture, alternating (Naive RAG, Cached RAG, Grep search, Hybrid RAG, Naive RAG, Cached RAG, Grep search, Hybrid RAG, …) to control for API time-of-day variance. Report the median; record variance.
...
- All architectures access the corpus through Support Content's exposed tool (`vector_search` for Naive RAG / Cached RAG, `grep_corpus` for Grep search, `hybrid_search` for Hybrid RAG). None reads the corpus file directly.
...
- **Vector store:** ChromaDB or FAISS, persisted under `architectures/naive_rag/vector_store/` and `architectures/hybrid_rag/vector_store/` (gitignored).
...
- **Reranker (Hybrid RAG only):** `cross-encoder/ms-marco-MiniLM-L-6-v2`, self-hosted via `sentence-transformers`.
...
- `architectures/{naive_rag,grep_search,hybrid_rag}/README.md` — per-architecture design choices and "done" criteria.
