# Implement Architecture C — Grep

## Why

Architecture C is the contrarian heart of the experiment. Per `ARCHITECTURE_RATIONALE.md`, the "MCP/RAG vs CLI/grep" framing lives or dies on whether keyword search, wrapped as an LLM-callable tool, is actually competitive with semantic retrieval for this task class.
...
- Implement agent loop in `architectures/c_grep/agent.py` with a system prompt that instructs keyword extraction
- Smoke-test with 2-3 hand-written queries

## Impact

- **New code:** `architectures/c_grep/agent.py`, `tools.py`, `prompts.py`
...
- **Depends on:** `measurement/tokens.py` and `corpus/swiss_faq.md` being in place. Ideally implemented after Architecture A so the Booking Systems tools are reusable rather than re-implemented.

## Modularity constraint compliance — critical for C

This rule matters MORE for C than for A or E because naive "grep" implies shell access. The agent code in this change must NOT shell out to `grep` or read `corpus/swiss_faq.md` directly. The grep functionality is implemented inside Support Content's `grep_corpus()` tool body — it happens to do substring matching, but the agent only sees the tool's structured response.
