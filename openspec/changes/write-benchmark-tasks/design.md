...
- **flight_no disambiguation (added PR #8/9, §4.5 fixture-docs cleanup):** `flight_no` is NOT unique in `data/travel.sqlite` — each route designator recurs across ~61 dated instances. Tasks referencing a flight_no MUST include a disambiguator (booking, date, or ticket number) so the agent can resolve to a unique `flight_id`. Bare `flight_no` references are ambiguous against the data and produce non-deterministic answers. The fixture status labels ("Scheduled" / "Arrived" / "Cancelled") in `task_fixtures.json` refer to the specific fixture-linked instance, not the flight_no globally — see `task_fixtures.json` `_meta.flight_no_recurrence`. Reference pattern: TXN-002 ("my LX0086 leg on booking 3F0481") disambiguates via the booking; loose-coupled cancellation phrasings ("my flight LX0000 got cancelled") still ground because the customer narrative supplies the date/instance the agent should verify via `get_flight_status`.
