# Implement Architecture E — Hybrid RAG

## Why

Architecture E is the production-standard pattern mature teams converge on — vector + BM25 + reranking. Per `ARCHITECTURE_RATIONALE.md`, without E in the comparison the article is vulnerable to "you compared against a tutorial-grade strawman" pushback.

E is the real benchmark every alternative must beat, not naive A. An alternative that beats A but loses to E is not actually a win — it's just better than the tutorial pattern.

## What changes

- Build BOTH a vector store (BGE-M3 embeddings, same as A) and a BM25 index over the same chunked corpus
...
- Implement agent loop in `architectures/e_hybrid_rag/agent.py`
- Smoke-test with 2-3 hand-written queries

## Impact

- **New code:** `architectures/e_hybrid_rag/agent.py`, `tools.py`, `prompts.py`, `setup_indices.py`
- **New persisted artifacts:** vector store + BM25 index under `architectures/e_hybrid_rag/vector_store/` (gitignored)
...
- **Depends on:** `measurement/tokens.py`, `corpus/swiss_faq.md`, ideally Architecture A done first so chunking/embedding pipeline is validated before adding BM25 and reranker on top
